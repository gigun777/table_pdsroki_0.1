  ...
  function openPicker({ title, items, onSelect, onAddCurrentLevel, getLabel }) {
    ...
    if (typeof onAddCurrentLevel === 'function') {
      modalChildren.push(h('button', {
        ...
        onClick: async () => {
          // Close current picker first, then open nested picker (e.g., template chooser).
          // Otherwise the nested modal gets closed immediately because it shares the same modal layer.
          closeModal();
  ...
  async function createJournalWithTemplate({ state, parentId, titlePrompt }) {
    ...
    function renderList() {
      ...
      const filtered = !query
        ? templates
        : templates.filter((t) => (`${t.title} ${t.id}`).toLowerCase().includes(query));
      for (const tpl of filtered) {
        list.append(h('button', { class: 'sdo-picker-row', onClick: () => onPick(tpl) }, [tpl.title]));
      }
      if (filtered.length === 0) {
        list.append(h('div', { class: 'sdo-picker-empty' }, ['Нічого не знайдено']));
      }
    }
    ...
    const spaceChildrenBtn = h('button', {
      ...
      onClick: () => openPicker({
        ...
        onAddCurrentLevel: async () => {
          ...
          const actNow = findById(stNow.spaces, stNow.activeSpaceId);
          if (!actNow?.id) return;
          await sdo.commit((next) => {
            next.spaces = [...next.spaces, { id: crypto.randomUUID(), title, parentId: actNow.id, childCount: 0 }];
          }, ['spaces_nodes_v2']);
        }
      })
